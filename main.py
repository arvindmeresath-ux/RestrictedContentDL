...
async def set_chat(bot: Client, message: Message):
    ...
    try:
        ...
        await message.reply(
            f"✅ **Forward chat changed successfully!**\n\n"
            f"**Name:** {chat.title}\n"
            f"**Chat ID:**\n{chat.id}"
        )

    except Exception as e:
        await message.reply(f"❌ Failed to set chat.\n\n`{e}`")
...
async def handle_download(bot: Client, message: Message, post_url: str):
    ...
    async with download_semaphore:
        ...
        try:
            ...
            if raw_caption:
                lines = raw_caption.splitlines()
                for i, line in enumerate(lines):
                    if "extracted" in line.lower():
                        raw_caption = "\n".join(lines[:i]).strip()
                        raw_caption_entities = None
                        break
...
async def queue_worker():
    while True:
        bot_client, msg, url, is_story = await DOWNLOAD_QUEUE.get()

        try:
            if is_story:
                await handle_story_download(bot_client, msg, url)
            else:
                await handle_download(bot_client, msg, url)
        except Exception as e:
            LOGGER(__name__).error(f"Queue Worker Error: {e}")
